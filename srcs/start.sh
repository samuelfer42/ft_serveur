#CERTIFICAT SSL
mkdir /etc/nginx/ssl
openssl req -newkey rsa:4096 -x509 -sha256 -days 365 -nodes -out /etc/nginx/ssl/localhost.pem -keyout /etc/nginx/ssl/localhost.key -subj "/C=FR/ST=Ile de France/L=Paris/O=42 School/OU=Gkatounk/CN=localhost"
...
echo "GRANT ALL ON *.* TO 'root'@'localhost' IDENTIFIED BY 'user42'" | mysql -u root
echo "FLUSH PRIVILEGES;" | mysql -u root
